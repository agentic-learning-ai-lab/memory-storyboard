...
from replay_buffer import MinRedBatchSampler
...
class Trainer():
    def __init__(self, args):
        ...
        self.longmem_dataset = StreamingDataset(dataset=args.dataset, scale_min=args.scale_min, scale_max=args.scale_max, ensure_overlap=args.ensure_overlap, subsample=args.dataset_subsample, class_length=args.class_length, crop_size=self.args.crop_size, return_indices=True)
    ...
    def train(self):
        ...
        self.longmem_sampler = MinRedBatchSampler(data_source=self.longmem_dataset, buffer_size=self.args.long_buffer_size, step_frame_size=self.args.curr_batch_size, batch_size=self.args.replay_batch_size, init_buffer_size=self.args.long_buffer_size)
        ...
        for step, (data_batch, labels, indices) in enumerate(self.longmem_dataloader):
            data_batch = data_batch.cuda()
            labels = labels.cuda()

            loss, log_dict,  feature1, feature2 = self.model(data_batch, labels, return_features=True)
            loss.backward()
            ...
            self.longmem_sampler.update_sample_stats({'idx': indices, 'feature1': feature1, 'feature2': feature2})
