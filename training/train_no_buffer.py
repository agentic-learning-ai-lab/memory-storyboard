...
from osiris_model import SimSiamSupCon, SimCLRSupCon
import wandb
import argparse
from datetime import datetime
from dataset_utils import *
from eval_model import eval_imagenet, eval_labeledS
from replay_buffer import SequentialBatchSampler
...
class Trainer():
    def __init__(self, args):
        ...
        if args.method == 'simclr':
            self.model = SimCLRSupCon(total_batch_size=args.curr_batch_size, replay_batch_size=0, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=0, tc_start_epoch=0, tc_curr_coef=0, temperature=0.1)
        elif args.method == 'simsiam':
            self.model = SimSiamSupCon(total_batch_size=args.curr_batch_size, replay_batch_size=0, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=0, tc_start_epoch=0, tc_curr_coef=0, temperature=0.1)
        else:
    ...
    def train(self):
        ...
        self.shortmem_sampler = SequentialBatchSampler(data_source=self.shortmem_dataset, step_frame_size=64, batch_size=self.args.curr_batch_size, subsample=self.args.subsample)
