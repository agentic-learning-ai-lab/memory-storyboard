...
from osiris_model import SimSiamSupCon, SimCLRTC, SimCLRSupCon, SimSiamTC
import wandb
import argparse
from datetime import datetime
from dataset_utils import *
from eval_model import eval_imagenet, eval_labeledS
from replay_buffer import IIDBatchSampler
...
class Trainer():
    def __init__(self, args):
        ...
        self.shortmem_dataset = StreamingDataset(dataset=args.dataset, scale_min=args.scale_min, scale_max=args.scale_max, ensure_overlap=args.ensure_overlap, subsample=args.dataset_subsample, class_length=args.class_length, crop_size=self.args.crop_size, label_path=args.label_path)

        if args.label_path is not None or args.class_length is None:
            if args.dataset == 'saycam':
                num_total_classes = 2000
            elif args.dataset == 'kcam':
                if args.label_path is not None:
                    num_total_classes = 1337
                else:
                    num_total_classes = 436
        else:
            num_total_classes = len(self.shortmem_dataset) // (args.class_length // args.dataset_subsample) + 1

        if args.method == 'tc':
            self.model = SimCLRTC(total_batch_size=args.curr_batch_size, replay_batch_size=args.curr_batch_size, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=0, tc_loss_coef=args.tc_loss_coef, num_total_classes=num_total_classes, tc_start_epoch=0)
        elif args.method == 'simclr_tc':
            self.model = SimCLRTC(total_batch_size=args.curr_batch_size, replay_batch_size=args.curr_batch_size, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=args.tc_loss_coef, num_total_classes=num_total_classes, tc_start_epoch=0)
        elif args.method == 'simclr':
            self.model = SimCLRSupCon(total_batch_size=args.curr_batch_size, replay_batch_size=args.curr_batch_size, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=args.tc_loss_coef, tc_start_epoch=0, tc_curr_coef=args.tc_curr_coef, temperature=0.1)
        elif args.method == 'simsiam_tc':
            self.model = SimSiamTC(total_batch_size=args.curr_batch_size, replay_batch_size=args.curr_batch_size, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=args.tc_loss_coef, num_total_classes=num_total_classes, tc_start_epoch=0)
        elif args.method == 'simsiam':
            self.model = SimSiamSupCon(total_batch_size=args.curr_batch_size, replay_batch_size=args.curr_batch_size, group_norm=args.group_norm, depth=args.depth, curr_loss_coef=args.curr_loss_coef, tc_loss_coef=args.tc_loss_coef, tc_start_epoch=0, tc_curr_coef=args.tc_curr_coef, temperature=0.1)
        else:
    ...
    def train(self):
        ...
        self.shortmem_sampler = IIDBatchSampler(data_source=self.shortmem_dataset, batch_size=self.args.curr_batch_size, step_frame_size=64)
        ...
        for step, (data_batch, labels) in enumerate(self.shortmem_dataloader):
            ...
            if (self.global_step+1) % self.args.eval_freq == 0 or self.global_step == len(self.shortmem_dataloader) - 1:
...
if __name__=='__main__':
    ...
    parser.add_argument('--curr_batch_size', default=256, type=int, help='Batch Size of Current Stream (default: 256)')
    ...
    parser.add_argument('--method', default='simclr_supcon', type=str, choices=['simclr', 'simsiam', 'osiris', 'tc', 'simclr_tc', 'simsiam_tc'], help='which method to use')
    parser.add_argument('--dataset', default='saycam', type=str, choices=['saycam', 'kcam'], help='which dataset to use')
    parser.add_argument('--label_path', default='', type=str, help='which dataset to use')
