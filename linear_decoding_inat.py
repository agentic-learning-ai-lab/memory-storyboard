...
from inat_dataset import INat2018

INAT_ROOT = 'path_to_inat2018'  # Change this to your iNaturalist dataset root directory
...
def load_split_train_test(datadir, args):
    ...
    train_data = INat2018(INAT_ROOT, split='train', transform=train_transforms)
    test_data = INat2018(INAT_ROOT, split='val', transform=test_transforms)
...
def main(args):
    ...
    optimizer = torch.optim.SGD(model.fc.parameters(), lr=12.0, momentum=0.9, weight_decay=0.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_dir', type=str, help='model checkpoint path')
    ...
    if "Saycam-SSL" in args.load_dir:
        args.name = 'inat' + model_ckpt_parts[-1][:-4]
    else:
        args.name = 'inat' + model_ckpt_parts[-2]
